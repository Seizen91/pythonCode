...
function parseQueryString() {
	...
	if (q && q.charAt(0)==='?') {
		...
		for (i=0; i<qs.length; i++) {
			...
			r[s.substring(0, pos)] = decodeURLComponent(s.substring(pos+1).replace(/\+/g, ' '));
		}
	}
	return r;
}
...
function Template(tpl) {
	var
		fn,
		match,
		code = ['var r=[];\nvar _html = function (str) { return str.replace(/&/g, \'&amp;\').replace(/"/g, \'&qout;\').replace(/\'/g, \'&#39;\').replace(/</g, \'&lt;\').replace(/>/g, \'&gt;\');};'],
	...
	$fn.$.extend({
		showFormError: function (err) {
			return this.each(function () {
				...
				if (! $form.is('form')) {
					console.log('Cannot call showFormError() on non-form object.');
					return;
				}
                ...
                if ($alert.length === 0) {
                	console.log('Cannot find .uk-alert-danger element.');
                	return;
                }
                if (err) {
...
function _httpJSON(method, url, data, callback) {
	...
	}).fail(function (jqXHR, textStatue) {
...
function getJSON(url, data, callback) {
	...
	if (typeof (data) === 'object') {
		...
		$.each(data, function (k, v) {
			arr.push(k + '=' + encodeURLComponent(v));
		});
		data = arr.join('&');
	}
